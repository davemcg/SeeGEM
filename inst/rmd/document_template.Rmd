...
DT::datatable(document_data$GEMINI_data %>% mutate_if(is.numeric, funs(as.numeric(formatC(., format = "e", digits = 2)))), #width=1200, 
  ...
  DT::formatStyle(columns = c('Mark1'), target = 'row', backgroundColor = DT::styleEqual(c(0, 1), c('#fcfcfa', '#ffd3d3'))) %>% 
  DT::formatStyle(columns = c('Mark2'), target = 'row', backgroundColor = DT::styleEqual(c(0, 1), c('#fcfcfa', '#e6ffd3')))
